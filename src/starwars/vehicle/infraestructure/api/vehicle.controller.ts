...
export default class VehicleController {
  ...
  async getAllVehicles(@Res() res: Response, @Query() query: GetAllVehiclesQuery){
    try {
      console.log('controller')
      const vehicleResult = await this.vehicleService.getAllVehicle(query);
  ...
  @ApiResponse({ status: 201, description: 'El vehiculo se creó con éxito.' })
  @ApiResponse({ status: 400, description: 'Bad request' })
  async createVehicle(@Body() createVehicleDto: CreateVehicleDto,  @Res() res: Response){
    try {
      ...
      return res.status(HttpStatus.CREATED).json(vehicleResult);
